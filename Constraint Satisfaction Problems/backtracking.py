...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    # print(unAssignedVars, csp, allSolutions, trace)
    ...
    if trace:
        print "==>Trying {}".format(nextVar.name())

    for val in nextVar.curDomain():

        if trace:
            print "==> {} = {}".format(nextVar.name(), val)
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    # constraints is a list. CSP 209: return list(self.constraints_of[i])
    while len(constraints) > 0:
        cnstr = constraints.pop()
        for nextVar in cnstr.scope():
            for val in nextVar.curDomain():
                if not cnstr.hasSupport(nextVar, val):
                    nextVar.pruneValue(val, reasonVar, reasonVal)
                    if nextVar.curDomainSize() == 0:
                        return "DWO"
                    for recheck in csp.constraintsOf(nextVar):
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nextVar.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(nextVar), csp, nextVar, val) == "DWO":

...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        # print(qi.name(), qj.name(), i, j)
        sa = []
        for columnI in range(0, qi.domainSize()):
            columnI += 1

            for columnJ in range(0, qj.domainSize()):
                columnJ += 1
                # check both column constraint and diagonal constraint
                if columnI != columnJ and abs(columnI-columnJ) != abs(i-j):
                    sa.append([columnI, columnJ])

        TableConstraint.__init__(self, name, [qi, qj], sa)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
	'''Return true if the values currently assigned to the variables in .scope() 
	satisfy the constraint.  False if not.  If .numUnassigned() > 0 return True
        (not yet able to check the constraint)'''
        count = 0
	for val in self.scope():
	...
	def nValsInBounds(l):
	    ...
	    varSetCount = 0
            for v in vals:
	        if v == self._required:
	            #The number of variables which are set to self._required
		    varSetCount+=1
	    return (varSetCount + (len(self.scope()) - len(vals))) >= self._lb and varSetCount <= self._ub
	varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], nValsInBounds, nValsInBounds)
        return x
